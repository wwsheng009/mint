...
- `popover/`：基础气泡卡片已经可用，auto + top/bottom 6 方位 placement、outside-click / ESC 收口和共享定位层已补齐；后续可继续补 left/right 系列 placement 与可组合内容区域
- `popconfirm/`：基础确认气泡已经可用，top/bottom 系列 placement、outside-click/ESC 收口、confirm/cancel variant、footer layout 和共享定位层已补齐；后续可继续补更丰富内容模板
...
- registry 细节已下沉到独立 compat 文件，核心上下文路径不再混入 registry 存取实现
