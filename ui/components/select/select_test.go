...
func TestVNode_IsFocusable(t *testing.T) {
	// Enabled select with options is focusable
	s := New().AddOption("a", "A")
	if !s.IsFocusable() {
		t.Error("Enabled select with options should be focusable")
	}

	// Disabled select is not focusable
	s = New().AddOption("a", "A").SetDisabled(true)
	if s.IsFocusable() {
		t.Error("Disabled select should not be focusable")
	}

	// Empty select is not focusable
	s = New()
	if s.IsFocusable() {
		t.Error("Empty select should not be focusable")
	}
}
...
func TestVNode_GetFocusID(t *testing.T) {
	s := New()
	s.SetKey("mykey")
	if s.GetFocusID() != "select:mykey" {
		t.Errorf("FocusID = %q, want %q", s.GetFocusID(), "select:mykey")
	}

	s = New()
	if s.GetFocusID() != "select" {
		t.Errorf("FocusID = %q, want %q", s.GetFocusID(), "select")
	}
}
