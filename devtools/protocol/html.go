...
// getInspectorHTML returns the debug inspector HTML.
func getInspectorHTML() string {
	return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        button { background: #0e639c; border: none; color: white; padding: 8px 16px; border-radius: 4px; cursor: pointer; margin-right: 10px; }
...
<body>
    <div class="container">
        ...
        <div class="panel">
            <h2>差异对比</h2>
            <input type="number" id="fromFrame" placeholder="起始帧 (例如: 0)" value="0" style="padding: 8px; background: #333; border: 1px solid #3e3e42; color: #d4d4d4; margin-right: 10px;">
            <input type="number" id="toFrame" placeholder="结束帧 (例如: 9)" value="9" style="padding: 8px; background: #333; border: 1px solid #3e3e42; color: #d4d4d4; margin-right: 10px;">
    ...
    <script>
        ...
        function displaySnapshots(snapshots) {
            ...
            if (!snapshots || snapshots.length === 0) {
                container.innerHTML = '<p>暂无快照</p>';
                return;
            }
            let html = '';
            for (const snap of snapshots) {
                html += '<div class="snapshot-card">';
                html += '<h3>帧 #' + snap.frame_id + '</h3>';
                html += '<small>ID: ' + snap.id + '</small><br>';
                html += '<small>组件数: ' + snap.components + '</small>';
                html += '</div>';
            }
        ...
        function displayDiff(payload) {
            ...
            if (!payload || !payload.changes || payload.changes.length === 0) {
                diffDiv.innerHTML = '<p>无差异</p>';
                return;
            }
            let html = '';
            for (const change of payload.changes) {
                if (change.type === 'added') {
                    html += '<div class="diff-added">+ ' + (change.node_id || change.NodeID) + ': 已添加</div>';
                } else if (change.type === 'removed') {
                    html += '<div class="diff-removed">- ' + (change.node_id || change.NodeID) + ': 已删除</div>';
                } else if (change.type === 'modified') {
                    html += '<div class="diff-changed">~ ' + (change.node_id || change.NodeID) + ': ' + change.path + '</div>';
                }
            }
