...
// TestLayerManagerGetLayout tests getting specific layer layout
func TestLayerManagerGetLayout(t *testing.T) {
	m := NewManager()

	// Get non-existent layer
	layout, ok := m.GetLayout(rtui.LayerBase)
	if ok {
		t.Error("GetLayout should return false for non-existent layer")
	}
	if layout != nil {
		t.Error("GetLayout should return nil for non-existent layer")
	}
	...
	layout, ok = m.GetLayout(rtui.LayerBase)
	if !ok {
		t.Error("GetLayout should return true for existing layer")
	}
	if layout != baseLayout {
		t.Error("GetLayout should return the correct layout")
	}
}
...
// TestLayerManagerContainsPosition tests position containment
func TestLayerManagerContainsPosition(t *testing.T) {
	m := NewManager()

	// Create a box from (10, 10) to (30, 30)
	root := &compute.ComputedBox{
		Box: runtime.Box{
			X:      10,
			Y:      10,
	...
	if !m.containsPosition(root, 15, 15) {
		t.Error("Should contain position (15, 15)")
	}

	// Contains position on edge
	if !m.containsPosition(root, 10, 15) {
		t.Error("Should contain position (10, 15) on left edge")
	}
	if !m.containsPosition(root, 29, 15) {
		t.Error("Should contain position (29, 15) near right edge")
	}

	// Does not contain position outside
	if m.containsPosition(root, 5, 15) {
		t.Error("Should not contain position (5, 15)")
	}
	if m.containsPosition(root, 35, 15) {
		t.Error("Should not contain position (35, 15)")
	}

	// Test with child box
	child := &compute.ComputedBox{
		Box: runtime.Box{
			X:      15,
			Y:      15,
			Width:  10,
	...
	root.Children = []*compute.ComputedBox{child}

	// Should contain position in child
	if !m.containsPosition(root, 20, 20) {
		t.Error("Should contain position (20, 20) in child")
	}
}

// TestLayerManagerCenterModal tests modal centering
func TestLayerManagerCenterModal(t *testing.T) {
	m := NewManager()
	...
	m.centerModal(root, constraints)

	// Modal should be centered: (100-10)/2 = 45
	if root.Box.X != 45 {
		t.Errorf("Modal X should be 45, got %d", root.Box.X)
	}
	if root.Box.Y != 45 {
		t.Errorf("Modal Y should be 45, got %d", root.Box.Y)
	}

	// Test with larger modal (offset should be 0)
	root.Box.X = 0
	root.Box.Y = 0
	root.Box.Width = 150
	root.Box.Height = 150
	...
	if root.Box.X != 0 {
		t.Errorf("Large modal X should be 0, got %d", root.Box.X)
	}
	if root.Box.Y != 0 {
		t.Errorf("Large modal Y should be 0, got %d", root.Box.Y)
	}
}

// TestLayerManagerShiftPositions tests position shifting
func TestLayerManagerShiftPositions(t *testing.T) {
	m := NewManager()
	...
	m.shiftPositions(root, 10, 20)

	// Root should be shifted by (10, 20)
	if root.Box.X != 20 {
		t.Errorf("Root X should be 20, got %d", root.Box.X)
	}
	if root.Box.Y != 30 {
		t.Errorf("Root Y should be 30, got %d", root.Box.Y)
	}

	// Child should also be shifted
	if root.Children[0].Box.X != 15 {
		t.Errorf("Child X should be 15, got %d", root.Children[0].Box.X)
	}
	if root.Children[0].Box.Y != 25 {
		t.Errorf("Child Y should be 25, got %d", root.Children[0].Box.Y)
	}
}

// TestLayerManagerPositionInspector tests inspector positioning
func TestLayerManagerPositionInspector(t *testing.T) {
	m := NewManager()
	...
	elem := ui.NewElement("inspector")
	elem.SetProps(ui.Props{
		"x": 50,
		"y": 75,
	})
	node := &LayerNode{
	...
	m.positionInspector(node, root)

	// Inspector should be positioned at (50, 75)
	if root.Box.X != 50 {
		t.Errorf("Inspector X should be 50, got %d", root.Box.X)
	}
	if root.Box.Y != 75 {
		t.Errorf("Inspector Y should be 75, got %d", root.Box.Y)
	}

	// Test with default position (no props)
	root.Box.X = 0
	root.Box.Y = 0
	elem2 := ui.NewElement("inspector")
	elem2.SetProps(ui.Props{})
	node.Content = elem2

	m.positionInspector(node, root)

	if root.Box.X != 0 {
		t.Errorf("Inspector X should be 0 by default, got %d", root.Box.X)
	}
	if root.Box.Y != 0 {
		t.Errorf("Inspector Y should be 0 by default, got %d", root.Box.Y)
	}
}
