...
func (s *UIScheduler) ScheduleUpdate(fiber *Fiber, lane Lane) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *UIScheduler) ScheduleFiberTree(root *Fiber, lane Lane) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func laneToPriority(lane Lane) priority.DirtyLevel {
	// Map lanes to priority levels
	if lane&LaneSyncLane != 0 {
		return priority.DirtyHigh
	}
	if lane&LaneInputContinuousLane != 0 {
		return priority.DirtyHigh
	}
	if lane&LaneDefaultLane != 0 {
...
func (w *WorkLoop) Schedule(fiber *Fiber, lane Lane) {
...
func (w *WorkLoop) SetRoot(fiber *Fiber) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.rootFiber = fiber
}
...
func (w *WorkLoop) Invalidate(lane Lane) {
	w.mu.RLock()
	root := w.rootFiber
	w.mu.RUnlock()
...
func (r *DefaultFiberRenderer) Layout(fiber *Fiber) {
	if r.onLayout != nil {
		r.onLayout(fiber)
	}
}
...
func (r *DefaultFiberRenderer) Paint(fiber *Fiber) {
	if r.onPaint != nil {
		r.onPaint(fiber)
	}
}
